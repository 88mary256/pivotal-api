...
@when("I get all epics of project with id {id} and filter {filter}")
def step_get_all_epic_of_project(context, id, filter):
...
def step_validate_epics_list(context):
    expect(context.all_epics_response.status_code)== 200
...
def step_validate_epic(context):
    expect(context.epic_response.status_code) == 200
